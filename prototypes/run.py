...
def main() -> None:
    ...
    parser.add_argument(
        "--capture",
        action="store_true",
        help="Use tdcv2.capture() replay instead of eager/dynamo execution.",
    )
    parser.add_argument(
        "--capture-wrapper",
        action="store_true",
        help="Pass wrapper=True to tdcv2.capture (default: wrapper=False, direct replay).",
    )
    parser.add_argument(
        "--export-metrics",
        action="store_true",
    ...
    if m.dynamo:
        ...
        mode = "capture v2 (direct)" if not args.capture_wrapper else "capture v2 (wrapper)"
    ...
    if args.capture:
        ...
        raw_model, example_inputs = m.get_module()
        if isinstance(example_inputs, dict):
            keys = tuple(example_inputs.keys())
            values = tuple(example_inputs.values())

            def _capture_fn(*args):
                return raw_model(**dict(zip(keys, args)))

            capture_args = values
        else:
            _capture_fn = raw_model
            capture_args = tuple(example_inputs)
        with torch.no_grad():
            captured = tdcv2.capture(
                _capture_fn, *capture_args, wrapper=args.capture_wrapper,
            )
        test = lambda: captured(*capture_args)
