...
### 3.1 每后端的 fallback wrapper 子类 + 注册表
- 每个设备一个薄子类,干净继承该设备**当前上游**的 wrapper 基类,只覆写 `create`
  (基类硬编码 `return <Base>()`,子类必须覆写才能被实例化):
  - `CppWrapperFallbackCpu(CppWrapperCpu)`
  - `CppWrapperFallbackGpu(CppWrapperGpu)`(cuda / xpu 共用)
- 它们刻意不加别的:全量 fallback 下没有融合核,基类的设备处理(CPU 直调;GPU 的
  stream / device guard / cuda c-shim 命名;Triton 启动机制休眠)正好够用。
- 注册表 `_FALLBACK_WRAPPER_BY_DEVICE = {"cpu": ..., "cuda": ..., "xpu": ...}`,
  context 按 device 选类。`CppWrapperFallback` 保留为 `CppWrapperFallbackCpu` 的别名。
- **新增一个后端的改动量 = 一个 ~5 行子类(继承该设备上游 wrapper + 覆写 create)
  + 一条注册表项**。其余(backend / context / benchmark)设备无关、一行不改。
- 接口:标准 wrapper codegen 接口(全继承)。依赖:对应设备的上游 wrapper 基类。
- NPU 注意:应注册一个继承**上游 `CppWrapperCpu`** 的干净子类,而非 torch_npu 现有
  的 stale `CppWrapperNpu`,以免继承其版本漂移缺陷。
...
- 职责(上下文管理器,`try/finally` 全恢复):
  ...
  - 按 device 从 `_FALLBACK_WRAPPER_BY_DEVICE` 取对应子类,snapshot 并 swap
    `device_codegens[device].cpp_wrapper_codegen = wrapper_cls`;
