...
唯一形态:`tdcv2.capture(fn, *example_args, **example_kwargs, allow_grad=False)`,
返回一个 callable,后续调用绕过 Dynamo/AOT runtime,直接走 C++ replay。
...
out = captured(x_real)                       # 直接 C++ replay
```

`allow_grad=True` 路径:fw + bw 双图捕获,封装在 `torch.autograd.Function`
里,自然 closure 形式即可(无需 functional_call):

```python
model = nn.Sequential(nn.Linear(8, 16), nn.ReLU(), nn.Linear(16, 4))

def train_step(x, y):
    return F.cross_entropy(model(x), y)      # 闭包引用 model
...
for x, y in batches:
    opt.zero_grad()
    captured(x, y).backward()                # grad 自动回到 model.parameters().grad
    opt.step()                               # in-place .data 修改通过 trace 的
                                             #   captured_tensors_ identity 自动可见
```

Dynamo 自动 lift 出来的 nn.Module 参数会作为 `_CapturedFn.apply` 的 leaf
args 暴露给 autograd(详见 §17.6.16),BN 等 mutated buffer 通过 AOT
`ViewAndMutationMeta` 在 forward 末尾被 write-back(详见 §17.6.17)。

历史曾存在的 `@tdcv2.compile` 装饰器(`torch.compile + aot_autograd + 自定义
compiler` 的封装)在 PoC 阶段实测严格慢于"dynamo eager backend",已经移除,
唯一推荐入口就是 `tdcv2.capture()`(参见 `python/v2/__init__.py` 顶部 docstring)。
...
|---|---|---|
| 触发方式 | `with tdc.capture():` 显式 | `tdcv2.capture(fn, *example_args, allow_grad=...)`,内部一次 `torch.compile(backend=aot_autograd(fw_compiler=...))`,后续调用绕过 Dynamo/AOT runtime |
| 动态 shape 支持范围 | dim-index 类 ops + `view(-1, ...)` | **完整动态 shape**(含 shape-derived literal) |
| 反向支持 | 实验性 `allow_grad=True`,warmup 必须 | `allow_grad=True`:fw + bw 双图捕获,包装在 `torch.autograd.Function` 内;闭包-lift 的 `nn.Parameter` 自动作为 autograd leaf 路由 grad(§17.6.16);mutated 输入(BN 等)通过 AOT `ViewAndMutationMeta` 在 fw 末尾 write-back(§17.6.17) |
| 工程量 | 已完成,~600 LoC | ~1100 LoC 增量 |
| 性能边界 | LLM decode / 小算子 / 固定 shape 训练 | 同 v1 + 通用动态 shape 推理 + 含 BN 的训练 |
| 与 PyTorch 的耦合 | 低(只用 dispatcher) | 中(依赖 Dynamo/AOT 接口稳定性,含 `TracingContext.fw_metadata` 这条 underscore-prefixed 但稳定的契约) |
...
**跨版本验证(2026-06-01,torch 2.13.0.dev / 2.9.0)**

本节的原始 dump 来自旧版 torch + NPU。在新版本上重新验证后,结论有更新:

1. **`disable_functionalization` 是 2.9 之后才新增的参数**,不是被移除的。
   - 2.9:`aot_module_simplified()` 无此参数,传入报
     `TypeError: unexpected keyword argument 'disable_functionalization'`。
   - 2.13:已是 `AOTConfig` 一等字段,贯穿 `aot_function` /
     `aot_module_simplified`,在 `_aot_autograd/graph_capture.py` 多处
     真正 gate functionalize 步骤。这是 torch 的正向演进方向。

2. **CPU 上的初步观察(不能外推到 NPU)。** 用 RMSNorm 形态(`x.float()` +
   标量 `eps` + `type_as`)在 **CPU + torch 2.13** 实测,`disable_functionalization`
   开/关、`dynamic` True/False 四种组合下:
   - `prims.convert_element_type` 计数恒为 **0**(dtype 转换以 `aten._to_copy`
     出现);标量 eps 保持内联 `Scalar`,未被 lift 成 0-d cpu tensor。
   - **但本节原始问题是 NPU 特有的**(prim 的 Python decomposition 跳板 +
     CPU 标量与 NPU tensor 的 H2D 同步),CPU 上本就不会暴露 device 相关的代价。
     因此该 CPU 结果**既不能证明 NPU 上也无 prim,也无法区分"上游已修"
     与"`disable_functionalization` 生效"两种可能**。
   - **待办:在 NPU + 2.13 上重测**(分别开/关 `disable_functionalization`),
     才能对"prim 是否仍出现、由谁消除"下结论。

3. **`rewrite_prims_in_gm` 保留**:幂等、无匹配即空转,作为跨版本廉价保险
   (2.9 / 旧版行为不一致;prim 仍可能从自定义 decomposition table 等其他
   来源冒出;NPU 行为尚未在 2.13 上复核)。**不要依赖它,也不要删。**

4. **2.9 的退化只在 slice_scatter 那一类**(依赖 `disable_functionalization`,
   2.9 缺失)。对策:特性探测,2.13+ 走官方参数,2.9 走 `make_fx`
   (默认不 functionalize,标量 lift 与 prim 均不产生,等效)回填。prim 专项
   修复(本节 ③)不受影响——它是 post-AOT FX pass,与版本无关。
...
## 18. inductor fx_wrapper 的交互点(v4 背景)

inductor 有三种 wrapper codegen 后端,产出同一份 Wrapper IR 的不同载体:
`PythonWrapperCodegen`(Python 文本)、`CppWrapperCpu/Gpu`(C++ 文本)、
`WrapperFxCodegen`(一张 `torch.fx.GraphModule`)。v4 通过 fx_wrapper 抓取 inductor
编译后的宿主图,需要清楚它和两侧的交互时机。**关键认知:WrapperFxCodegen 与
Scheduler 的交互和普通 PythonWrapperCodegen 完全一样——都是"累积 Wrapper IR
Line";FX 转换不在调度期,而是调度后由 `FxConverter` 单独跑一遍。**

总时序(`GraphLowering.codegen`):
```
init_wrapper_code()      创建 WrapperFxCodegen(空 self.lines)
scheduler.codegen()      交互面 A:scheduler 往 wrapper 累积 Wrapper IR Line
wrapper_code.generate()  交互面 B:Line -> FX 图,产出返回给上层的结果
```

### 18.1 交互面 A:Scheduler -> WrapperFxCodegen(调度期)

时机:`Scheduler.codegen()` 遍历每个 SchedulerNode 时。机制:scheduler / 后端
Scheduling / IR 节点调 wrapper 的 `codegen_*` / `generate_*` / `writeline`,每次往
`self.lines` 追加一个 `WrapperLine`。**此阶段不产生任何 FX 节点。**

| scheduler 调的函数 | 传入 | 效果 | WrapperFxCodegen 覆写 |
|---|---|---|---|
| `codegen_allocation(buffer)` | buffer IRNode | 追加 `AllocateLine` | 否 |
| `codegen_free` / `codegen_inplace_reuse` | buffer | free/reuse Line | 否 |
| `define_kernel(name, src, ...)` | Triton 核名+源码 | kernel 定义 Line | `_define_kernel_helper` |
| `generate_kernel_call(...)` | 核名/参数/grid | `KernelCallLine` | 否 |
| `FallbackKernel.codegen(wrapper)` | extern/fallback 节点 | `ExternKernel*Line` | 否 |
| `codegen_conditional(cond)` | Conditional 节点 | `ConditionalLine` + 递归子图 | 是 |
| `define_subgraph_launcher_fn(name, code)` | 子图 GraphModule | 存入 `self.subgms` | 是 |
| `codegen_inputs()` | - | no-op(输入交给 FxConverter) | 是 |

WrapperFxCodegen 只覆写少数(conditional / subgraph / inputs no-op),其余继承——
因为它们产出标准 Wrapper IR Line,FX 转换推迟。这正是 scheduler 不知道下游是
python/cpp/fx 的原因。

### 18.2 调度后转换:`wrapper.generate` -> `FxConverter`

时机:`scheduler.codegen()` 结束后,`GraphLowering.codegen` 调
`wrapper_code.generate(is_inference)`。

| 函数 | 传入 | 返回 |
|---|---|---|
| `WrapperFxCodegen._generate(is_inference)` | `is_inference`;读 `self.lines` / `get_fx_graph_inputs()` / `get_graph_outputs()` / `self.subgms` | `(FileBackedGraphModule(gm, compiled_fn), None)` |
| `run_wrapper_ir_passes(is_inference)` | - | 对 `self.lines` 跑 Wrapper IR 优化 |
| `FxConverter(...).generate()` | Lines + 输入输出 + 子图 | `torch.fx.GraphModule` gm |
| `compile_graph(gm)` | gm | `compiled_fn`(默认 `gm.forward`) |

`FxConverter.generate()` 逐 Line 调 `line.codegen_fx(self)(line)`,映射到
`_generate_*` 把每个 Line 变成 FX 节点:`AllocateLine -> empty_strided`;
`KernelCallLine -> wrap_triton + triton_kernel_wrapper_mutation HOP`;
`ExternKernel*Line -> aten OpOverload`。最后 `_generate_outputs()` +
`gm.graph.output(...)` + `gm.recompile()` -> 返回 gm。

### 18.3 交互面 B:WrapperFxCodegen -> 返回给 torch.compile 的结果

`_generate` 返回的 `FileBackedGraphModule(gm, compiled_fn)` 沿
`_compile_to_module` -> `compile_fx` -> AOTAutograd -> Dynamo 上交:

| 环节 | 函数 | 传入 | 返回 |
|---|---|---|---|
| inductor 收产物 | `_compile_to_module` | `FileBackedGraphModule` | 当作 `mod`(伪装成 ModuleType) |
| 暴露代码文本 | `save_output_code(wrapper_code.value)` | `wrapper_code.value = gm.code` | 即 `run_and_get_code` / `TORCH_LOGS=output_code` 拿到的文本 |
| **唯一拦截点** | `compile_graph(gm)` | **gm** | 运行期 callable(默认 `gm.forward`) |
| 上交 | compile_fx 返回 | `gm.forward` | 被 AOTAutograd RuntimeWrapper 再包 -> Dynamo 缓存 |

**两个边界的本质**:下边界(scheduler)时 gm 还不存在,scheduler 只写设备无关的
Wrapper IR Line;上边界时 gm 在 `FxConverter.generate()` 才诞生,经
`compile_graph(gm)` 变成运行期 callable 上交。**gm 对象只在 `compile_graph` 这一个
时机暴露**——所以从上层 compiled 对象拿不到 gm 对象,要拿必须在 `compile_graph`
